...
public class HorarioDAO {
    ...
    public List<String> listarHorasDisponibles(int idMedico, String fechaIso, int idCitaExcluir) throws SQLException {
        ...
        int duracionCita = obtenerDuracionCita(idMedico);
        Set<String> horasGeneradas = generarBloquesHora(bloques, duracionCita);
    ...
    private Set<String> generarBloquesHora(List<LocalTime[]> bloques, int duracionCitaMinutos) {
        Set<String> horas = new LinkedHashSet<>();
        int paso = duracionCitaMinutos <= 0 ? 30 : duracionCitaMinutos;
    ...
    /** Obtiene la duración configurada de cada cita para generar horarios reales. */
    private int obtenerDuracionCita(int idMedico) throws SQLException {
        String sql = "SELECT COALESCE(duracion_cita_minutos, 30) AS duracion FROM medico WHERE id_medico=?";
        try (Connection connection = ConexionDB.getConnection(); PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, idMedico);
            try (ResultSet rs = statement.executeQuery()) {
                if (rs.next()) {
                    int duracion = rs.getInt("duracion");
                    return duracion <= 0 ? 30 : duracion;
                }
            }
        }
        return 30;
