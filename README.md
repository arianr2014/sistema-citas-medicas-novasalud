# Sistema de Citas Médicas NovaSalud - V3.2.1

## 1. Descripción general

El Sistema de Citas Médicas NovaSalud V3.2.1 es una aplicación web desarrollada en Java para la gestión integral de consultas externas.

Permite administrar pacientes, médicos, especialidades, horarios, citas médicas, pagos, caja, agenda médica, reportes, estadísticas, usuarios e historia clínica.

La versión V3.2.1 incorpora mejoras de integridad de datos, seguridad de autenticación, control de acceso por roles, registro clínico por atención médica e historia clínica asociada al DNI del paciente.
...
Base de datos: bd_citasmedicas_v321
Host: localhost
Puerto: 3306
```
...
```text
jdbc:mysql://localhost:3306/bd_citasmedicas_v321?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC
```
...
6. Ingresar como `recepcionista1`.
7. Ingresar como `cajero1`.
8. Ingresar como un médico (ej. `camila.torres`).
...
- Cada usuario DOCTOR debe estar asociado a un médico activo.
- La historia clínica base se genera automáticamente al crear pacientes.

### 10.7. Registro clínico por atención médica
